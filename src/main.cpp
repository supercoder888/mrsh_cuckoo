...
void
mrsh_CF::readFingerprint(){
    filter.readFingerprint();
}

void 
mrsh_CF::writeFingerprint(){
...
mrsh_CF::bufferInCuckooFilter(unsigned char *byte_buffer, uint64_t bytes_read, std::string filename){
    ...
    for (i=0; i<bytes_read; i++){
        ...
        if (rValue %BLOCK_SIZE ==  BLOCK_SIZE -1) {
            ...
            char *buff = new char[block_size+1];
            copyBlockToBuffer(byte_buffer, buff, last_block_index, i);
            //generateIndexTagHash(buff, block_size, &hv);

            for(j=last_block_index;j<=i;j++){
                buff[j-last_block_index] = byte_buffer[j];
            }
    ...
    if(chunkCount+1 != chunkDetected){
        std::cout<< filename<<  "\t Total Chunks: "<< chunkCount+1 <<  "\t Chunks Detected: " << chunkDetected<<"\n";
    }
...
int main(int argc, char **argv){
    try{
        ...
        desc.add_options()
            ...
            ("signature,s", "Load signatures in file 'mrsh.sig'")
            ("generate,g", "Generate signature 'mrsh.sig' for file given with -f")
